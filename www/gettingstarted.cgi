...
<p>The first step is <a href="/connection.cgi">Connection and authentication</a> which allows you set up the connection URLs, client IDs, and client secrets, for messages each way, and your details.</p>
<p>Once you have done that you can start sending messages each way. One useful trick is to set your system to route messages to an RCPID of <tt>TEST</tt> here, allowing you to send and receive test messages even when you are set up on TOTSCO pre-production or live.</p>
